...
parser = argparse.ArgumentParser(
    description="Device Shadow sample keeps a property in sync across client and server"
)
parser.add_argument(
    "--endpoint",
    required=True,
...
parser.add_argument(
    "--thing-name", required=True, help="The name assigned to your IoT Thing"
)
parser.add_argument(
    "--shadow-property",
    default="color",
    help="Name of property in shadow to keep in sync",
)
parser.add_argument(
    "--use-websocket",
    default=False,
...
parser.add_argument(
    "--proxy-port", type=int, default=8080, help="Port of proxy to connect to."
)
parser.add_argument(
    "--verbosity",
...
def user_input_thread_fn():
    ...
    while True:
        try:
            ...
            if temp > 130:
                time.sleep(3)
                exit("We've hit the end")
                break
            else:
                change_shadow_value(temp)
                time.sleep(2)
        ...
        with locked_data.lock:
            ...
            publish_get_future = shadow_client.publish_get_shadow(
                request=iotshadow.GetShadowRequest(
                    thing_name=args.thing_name, client_token=token
                ),
                qos=mqtt.QoS.AT_LEAST_ONCE,
            )
